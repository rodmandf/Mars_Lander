...
void PhysicsEngine::update(ControlOutput input, float terrainHeight) {
    ...
    // 1. Перекачка топлива (если в главном < 100, берем из запасных)
    ...
    if (state.y >= terrainHeight - groundoffset) { 
        ...
        bool safeSpeed = std::abs(state.vy) < 10.0f && std::abs(state.vx) < 10.0f;
        bool safeAngle = std::abs(state.angle) < 0.3f;

        if (safeSpeed && safeAngle) state.landed = true;
